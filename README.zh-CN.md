...
- `skills/ai-native-model-router/` v0.1.0：可选的路由扩展（routing-extension）。
...
| `ai-native-model-router` | 0.1.0 | routing-extension | 通过 `route/v1` 和 `.ai-native/model-router.json` 确定具体供应商和模型（provider/model）。它不执行宿主。 |
...
任务
  -> Team 的语义 route slot
  -> 可选的 Router：通过 route/v1 返回 RouteDecision
  -> Host Adapter
```

Team 先给出语义路由槽位（`route slot`），即使没有 Router 也能继续工作。没有 Router 时，供应商和模型的映射保持 `unknown` 或由宿主继承，不能靠推测补齐。Router 只为宿主适配器（Host Adapter）返回决策，不声称已经调用供应商或强制执行了路由。
...
```text
使用 $ai-native-dev-team 和 $ai-native-model-router。先检查任务，生成语义 route slot；只有在收到明确的配置档（profile）和可用性（availability）输入后才通过 route/v1 解析，并把宿主执行保持在独立边界。现在不要修改文件。
```

## 配置 Router

在项目中创建 `.ai-native/model-router.json`，内容使用下面这个完整的 v1 结构。`active_profile` 必须显式选择；仅有配置文件或 profile 文件不会自动启用路由。配置中不放密钥或其他敏感信息。
...
参见[配置 Schema](skills/ai-native-model-router/assets/model-router-config.v1.schema.json)和[示例](skills/ai-native-model-router/assets/model-router-config.example.json)。仅改配置只适用于已有的宿主适配器合同。新增认证方式、传输机制或宿主注入方式时，仍需修改适配器代码；Router 不会凭配置增加这些能力。
...
- 默认路由配置用 GLM-5.3 做主要 `Validator`，用 GLM-5.3 Flash 做必须显式选择的高吞吐 `Writer`。只有主模型不可用且提供的证据被配置档接受时，才允许回退到 DeepSeek；可用性必须由调用方提供，不能推断。
