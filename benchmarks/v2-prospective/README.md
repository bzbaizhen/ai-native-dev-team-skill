...
- Store each referenced V1 baseline evidence object in the freeze Commit. Case-level evidence may be eligible for the stable five-task mechanism/quality gate while remaining ineligible for a formal efficiency claim; only set formal-efficiency eligibility when the required denominators are reconstructable.
...
The command rejects merge history, receipt edits/deletes, non-continuous registration, a self-rehashed replacement registry, unanchored `task_ready`, invented or digest-mismatched V1 baselines, source-level hard-gate violations in the candidate window, and a candidate Commit that never reached the stable Commit. It cannot prove that a local ref was remotely protected or when it was pushed; the trusted freeze/head pair must come from an independent control. It does not prove efficiency improvement. Formal claims still require 15–20 accepted tasks whose frozen V1 baselines explicitly have reconstructable denominators, plus the preregistered stratified comparison.
